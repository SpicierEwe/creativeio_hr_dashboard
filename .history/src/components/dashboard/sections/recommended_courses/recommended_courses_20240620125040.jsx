  ...
  return (
    <div className="p-5 mt-7  flex flex-col items-start bg-white rounded-xl overflow-x-hidden">
      ...
      <div className="flex justify-between items-start text-start w-full">
        <h2 className="w-[70%] md:w-auto">Recommended Courses</h2>
        <p className="cursor-pointer">See all {">"}</p>
      </div>

      {/* cards */}
      <div className="mt-10 w-full grid gid-cols-1  md:grid-cols-2 lg:grid-cols-34 flex-wrap gap-10">
        {coursesData.map((course) => (
          <div
            key={course.id}
            className="flex-1 flex flex-col border rounded-2xl"
          >
            <div className=" w-full flex flex-col h-full mx-auto text-start">
              {/* container 1 */}
