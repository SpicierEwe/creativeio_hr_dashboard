...
const Courses = () => {
  const [selectedNavIndex, setSelectedNavInde] = useState(0);
  return (
    <div className="p-5 mt-7 flex flex-col items-start bg-white rounded-xl">
      ...
      <div className="text-xs mt-5 flex gap-3">
        ...
        {nav.map((item, index) => (
          <button
            key={index}
            onClick={() => setSelectedNavInde(index)}
            className={`${
              selectedNavIndex === index
                ? "border-accent"
                : "border-b-transparent"
            } cursor-pointer  border-b-2 pb-5 transition-all duration-2300 ease-in-out`}
          >
            {item}
          </button>
        ))}
      ...
      <div className="mt-10 w-full flex flex-col gap-10">
        {coursesData.map((course) => (
          <div className=" rounded-2xl flex flex-col lg:flex-row   lg:items-center gap-4 text-start justify-between  ">
            {/* container 1 === */}
            <div className=" flex flex-col md:flex-row lg:items-center lg:w-[50%] gap-3 md:gap-7  ">
              {/* image */}
