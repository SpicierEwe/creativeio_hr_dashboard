function Section2() {
  ...
  return (
    <div>
      <div className="flex flex-col gap-5 bg-white p-10 shadow-lg">
        {data.map((item) => (
          <div>
            <h2 className="text-start font-medium">{item.title}</h2>
            <ul className="mt-5 grid grid-cols-4  grid-auto-flow-row dense items-center flex-wrap">
              {item.content.map((subItem, index) => (
                <li key={subItem.id} className="flex items-center">
                  <img
                    src={`/images/section-2/icons/${index + 1}.svg`}
                    className="mt-5 h-28 scale-150"
                  />
                  <div className="text-start">
                    <h2 className="">{subItem.title}</h2>
                    <p className="text-[#abafb3] mt-1">{subItem.subtitle}</p>
                  </div>
                </li>
              ))}
            </ul>
